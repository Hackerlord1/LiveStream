...
export function useIptvChannels(page = 0, genre?: string) {
  const fetch = useCallback(() => callWrapper("iptv:getOrderedList", { page, genre }), [page, genre]);
  return { fetch };
}
...
export function useIptvEpg(period = 1) {
  const fetch = useCallback(() => callWrapper("iptv:getEpgInfo", { period }), [period]);
  return { fetch };
}

export function useIptvShortEpg(channelId: number, size = 10) {
  const fetch = useCallback(() => callWrapper("iptv:getShortEpg", { channelId, size }), [channelId, size]);
  return { fetch };
}

export function useLiveGames(period = 1, page = 0, pageSize = 20) {
  const fetch = useCallback(() => callWrapper("iptv:getLiveGames", { period, page, pageSize }), [period, page, pageSize]);
  return { fetch };
}

export function useStreamUrl(channelId: number) {
  const fetch = useCallback(() => callWrapper("iptv:getLiveStreamUrl", { channelId }), [channelId]);
  return { fetch };
}